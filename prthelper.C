...
extern std::map<uint64_t, std::string> cgroup_paths;
...
void
prttspid(uint64_t ts, int p, uint64_t cgroup_id, uint64_t start_time, uint32_t sn, uint8_t procid, 
   ...
   if (longts) {
      fprintf(ofp, "%lu.%06lu:", ts / 1000000, ts % 1000000);
   } else {
      fprintf(ofp, "%lu.%03lu:", ts / 1000, ts % 1000);
   }
   ...
   if (useprocid) {
      fprintf(ofp, "%02x:", procid);
   }

    std::string cg_path = "";
    auto it = cgroup_paths.find(cgroup_id);
    if (it != cgroup_paths.end()) {
        cg_path = " cgroup_path=" + it->second;
    }
    fprintf(ofp, "  pid=%d cgroup=%lu%s start_time=%lu: ", p, cgroup_id, cg_path.c_str(), start_time);
}
...
prt_exitids(const char* scnm, int par_tid, int ret, int uid, int gid, 
            long cgroup, int pid, int tid, int last_pid, int last_tid,
             bool force_ts, FILE* ofp, bool is_tampered) {
   std::string cg_path = "";
   auto it = cgroup_paths.find((uint64_t)cgroup);
   if (it != cgroup_paths.end()) {
      cg_path = " cgroup_path=" + it->second;
   }
   if (force_ts || pid != last_pid || tid != last_tid) {
      fprintf(ofp, "%s ret=%d parent_tid=%d uid=%d gid=%d cgroup=%ld%s",
              scnm, ret, par_tid, uid, gid, cgroup, cg_path.c_str());
      if(is_tampered) fprintf(ofp," tampered=y");
   }
      

   else {
      fprintf(ofp, " ret=%d parent_tid=%d uid=%d gid=%d cgroup=%ld%s",
              ret, par_tid, uid, gid, cgroup, cg_path.c_str());
      if(is_tampered) fprintf(ofp," tampered=y");
