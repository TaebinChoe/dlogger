...
/* Maps */
volatile unsigned int dlogger_pid = 0;
...
static __always_inline bool should_trace() {
    ...
    if (dlogger_pid != 0) {
        ...
        for (int i = 0; i < 5; i++) {
            if (!task) break;
            p_pid = BPF_CORE_READ(task, tgid);
            if (p_pid == dlogger_pid) {
                return false;
            }
                    ...
                    if (file) {
                        ...
                        if (inode) {
                            ...
                            if (ev) {
                                ...
                                ev->start_time = BPF_CORE_READ(curr_task, start_time);
        ...
        if (fstate) {
            ...
            if (stats) {
                ...
                if (dxt) {
                    ...
                    dxt->length = length;
                    dxt->pid = pid;
                    dxt->cgroup_id = bpf_get_current_cgroup_id();
                    struct task_struct *curr_task = (struct task_struct *)bpf_get_current_task();
                    dxt->start_time = BPF_CORE_READ(curr_task, start_time);
